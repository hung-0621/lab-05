...
async function testRelations() {
  ...
  const student_id = "S10811001"; // 王小明
  const course_id = "CS101001"; // 計算機概論
  ...
  try {
    ...
    // 查詢學生及其選修的所有課程
    const studentWithCourses = await Student.findByPk(student_id, {
      include: [Course]
    });
    ...
    // 查詢課程及其選修的學生
    const courseWithStudents = await Course.findByPk(course_id, {
      include: [Student]
    });
